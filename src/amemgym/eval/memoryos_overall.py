"""Evaluation module for MemoryOS agent performance assessment."""
...
def evaluate_item(item, agent, output_dir, env_config, off_policy=False):
    """Evaluate MemoryOS agent on a single evaluation item."""
        ...
        if os.path.exists(agent_state_dir):
            ...
            if off_policy:
                ...
                for session, session_msgs in tqdm(zip(sessions, interactions), desc="Off-policy Interactions", leave=False):
                    ...
                    assert len(session_msgs) == 2 * max_rounds
                    for i in tqdm(range(0, 2 * max_rounds, 2), ncols=80, leave=False):
